...
abstract Member(Field) from Field to Field {
	...
	static public function setter(field:String, ?param = 'param', ?pos, e:Expr, ?t:ComplexType) {
		return method(
            ...
            [e, param.resolve(pos)]
              .toBlock(pos)
              .func([param.toArg(t)], t)
        );
    }

	static public function method(name:String, ?pos, ?isPublic = true, func:Function):Member {
		var field:Field = {
			name: name,
			pos: if (pos == null) func.expr.pos else pos,
			kind: FFun(func)
		};
		var ret:Member = field;
...
/* === [0] Properties === */
	...
	public function getVar(?pure = false) {
		return switch kind {
			...
			case FProp(get, set, t, e) if (!pure):
                Success({
                    get: get,
                    set: set,
                    type: t,
            ...
            default:
                pos.makeFailure('Field should be a variable ' + if (pure) '' else 'or property');
		}
    }
	...
	public inline function asField():Field {
		return this;
    }
	...
	inline function get_meta():Metadata {
	...
	inline function set_meta(param) {
	...
	inline function set_name(param) {
	...
	inline function set_doc(param)
		return this.doc = param;

	inline function get_kind()
		return this.kind;

	inline function set_kind(param)
		return this.kind = param;

	inline function get_pos()
		return this.pos;

	inline function set_pos(param)
		return this.pos = param;

	inline function get_overrides()
		return hasAccess(AOverride);

	inline function set_overrides(param)
		return setAccess(AOverride, param);

	inline function get_isStatic()
		return hasAccess(AStatic);

	inline function set_isStatic(param)
		return setAccess(AStatic, param);

    /* getter/setter for `isPublic` */
	...
	function hasAccess(a: Access):Bool {
		if (this.access != null)
	...
	function setAccess(a:Access, isset:Bool) {
		changeAccess(isset ? a : null, isset ? null : a);
		return isset;
	}
