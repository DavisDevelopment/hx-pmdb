...
#if macro
    /**
      will copy all fields from the type referred to by [typeId] that aren't already present on the type it is called on
     **/
    public static function buildCopyFrom(typeId: String):Array<Field> {
        var srcType = Context.getType(typeId);
        if (srcType == null)
            Context.fatalError('"$typeId" not found', Context.currentPos());
        ...
        switch ( srcType ) {
            case Type.TInst(_.get()=>src, params):
                //
                var srcFields = ClassFields.getClassFields(src, true, params);
                for (sf in srcFields) {
                    if (!fieldMap.exists(sf.name)) {
                        fieldMap[sf.name] = fields[fields.push( sf ) - 1];
                    }
                }
...
class ClassFields {
    ...
    static function getFieldVarAccess(field: ClassField):{get:String,set:String} {
        return switch field.kind {
            case FVar(r, w): {get:getVarAccess(r), set:getVarAccess(w)};
            default: throw 'nope';
        }
    }
                    ...
                    switch (methodKind) {
                        ...
                        default:
                            switch (Context.follow(field.type)) {
                                ...
                                default: 
                                    throw "not implemented for type [" + field.type + "]";
                            }
                    }
                }
        }
    ...
    static function convertTFunArgsToFunctionArgs(args : Array<{ t : Type, opt : Bool, name : String }>, paramMap:Array<TypeDeclaration>):Array<FunctionArg> {
        ...
        for (arg in args) {
            ...
            var value:Null<Expr> = null;//arg.opt ? arg.t.toComplexType().getDefaultValue() : null;
    ...
    static function getVarAccess(access:VarAccess):String {
        return switch (access) {
            ...
            case VarAccess.AccCtor: 'default';//throw "final";
        }		
    }
