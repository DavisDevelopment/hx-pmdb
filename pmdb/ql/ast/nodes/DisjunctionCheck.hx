...
class DisjunctionCheck extends Check {
...
/* === Methods === */
    ...
    override function clone():Check {
        return new DisjunctionCheck(left, right, expr, position);
    }

    override function map(fn: QueryNode->QueryNode, deep=false):QueryNode {
        return new DisjunctionCheck(safeNode(fn(left), Check), safeNode(fn(right), Check), expr, position);
    }
    ...
    override function equals(o: QueryNode):Bool {
        return (super.equals( o ) || (Arch.isType(o, mytype()) ? cast(o, DisjunctionCheck).passTo(c -> c.left.equals(left) && c.right.equals(right)) : false));
    }
    ...
    override function getExpr() {
        return PredicateExpr.POpBoolOr(left.getExpr(), right.getExpr());
    }
